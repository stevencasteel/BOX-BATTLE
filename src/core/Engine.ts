...
export class Engine {
   ...
   */
  private updateDirectDOMHUD(playerHP: number, bossHP: number, healingCharges: number, determination: number) {
    for (let i = 0; i < 5; i++) {
  ...
  private projectState() {
    ...
    this.updateDirectDOMHUD(nextPlayerHP, nextBossHP, nextHealingCharges, nextDetermination);
